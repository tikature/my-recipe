...
func homeHandler(w http.ResponseWriter, r *http.Request) {
	...
	if query == "" {
		...
		log.Printf("No query, showing all %d recipes", len(filteredResep))
	} else {
		for _, resep := range resepData {
			...
			if strings.Contains(strings.ToLower(resep.Nama), query) {
				...
				log.Printf("Found recipe by name: %s", resep.Nama)
				continue
			}
			...
			for _, tag := range resep.Tag {
				if strings.Contains(strings.ToLower(tag), query) {
					filteredResep = append(filteredResep, resep)
					log.Printf("Found recipe by tag: %s (tag: %s)", resep.Nama, tag)
					break
				}
			}
		}
		log.Printf("Search completed, found %d recipes", len(filteredResep))
	}
	...
	log.Printf("Sending %d recipes to template", len(data.Recipes))
...
func main() {
	// Debug: Print jumlah resep yang tersedia
	log.Printf("Starting server with %d recipes loaded", len(resepData))
	for i, resep := range resepData {
		log.Printf("Recipe %d: %s", i+1, resep.Nama)
	}
	...
	http.HandleFunc("/resep/", detailHandler)
